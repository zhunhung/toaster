...
def toast(method):
    if webhook_url == '':
        raise UnboundLocalError('You have not configured your webhook url. Run set_incoming_webhook(<webhook url>) first.')
    ...
    def insert_toast(*args, **kw):
        try:
            ...
            res = requests.post(url = WEBHOOK_URL, data = json.dumps(data), headers={'Content-Type': "application/json"})
            return result
